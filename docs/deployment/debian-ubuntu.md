...
The env file is created empty and never overwritten by Ansible. Add secrets on the target host after provisioning.
...
sudo install -d -o root -g agent -m 0750 /etc/homelab-agent
sudo install -o root -g agent -m 0640 /dev/null /etc/homelab-agent/foxhole.env
