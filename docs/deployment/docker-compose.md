...
The Compose stack runs the API, Celery worker, Celery beat, Redis, and a Docker socket proxy by default. The API is published on `127.0.0.1:8000`. Flower is debug-only and is published on `127.0.0.1:5555` only when the `debug` profile is enabled. The Docker socket proxy is on an internal-only network and has no host port.
