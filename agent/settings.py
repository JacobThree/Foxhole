...
class IntegrationSettings(BaseSettings):
    ...
    def configured(self) -> bool:
        return self.base_url is not None and (self.api_key is not None or self.token is not None)
...
class ProxmoxSettings(BaseSettings):
    ...
    @property
    def configured(self) -> bool:
        return self.host is not None and self.token_id is not None and self.token_secret is not None
...
class DockerSettings(BaseSettings):
    ...
    @property
    def configured(self) -> bool:
        return self.socket_proxy_url is not None
...
class AppSettings(BaseSettings):
    ...
    def docker(self) -> DockerSettings:
        return DockerSettings(socket_proxy_url=self.docker_socket_proxy_url)
    ...
    def plex(self) -> IntegrationSettings:
        return IntegrationSettings(base_url=self.plex_base_url, token=self.plex_token)

    @property
    def sonarr(self) -> IntegrationSettings:
        return IntegrationSettings(base_url=self.sonarr_base_url, api_key=self.sonarr_api_key)

    @property
    def radarr(self) -> IntegrationSettings:
        return IntegrationSettings(base_url=self.radarr_base_url, api_key=self.radarr_api_key)

    @property
    def tautulli(self) -> IntegrationSettings:
        return IntegrationSettings(base_url=self.tautulli_base_url, api_key=self.tautulli_api_key)
    ...
    def unbound(self) -> UnboundSettings:
        return UnboundSettings(host=self.unbound_host, port=self.unbound_port)
